...
// Digl's thoughts
/*
The PDF explains how boundaries are formed from having a ring of dead cells
around the active cells.

So, to create boundaries, you just have to kill some cells :)
...
public class NavierStokesSolver {
	...
	int[] deadCells = new int[SIZE];

	public NavierStokesSolver() {
		initialiseAliveCell();
		addDeadRegion(0, 0, 1, 25);
		addDeadRegion(0, 0, 25, 1);

		addDeadRegion(24, 0, 25, 25);
		addDeadRegion(0, 24, 25, 25);
	}
	// DIGL FUNCTIONS BEGIN
	private void initialiseAliveCell(){
		int k;
		for(k = 0; k<deadCells.length;k++){
			deadCells[k] = 1;
		}
	}

	private void addDeadRegion(int x, int y, int w, int h){
		for(int k = x; k<w; k++){
			for(int j = y; j<h; j++){
				addDeadCell(k, j);
			}
	...
	private void addDeadCell(int x, int y){
		deadCells[ y*(N+2) + x+1 ] = 0;
	}

	private void muteDeadCells(){
		int k;
		for(k = 0; k<deadCells.length;k++){
			u[k] *= deadCells[k];
			v[k] *= deadCells[k];
			u_prev[k] *= deadCells[k];
			v_prev[k] *= deadCells[k];
			dense[k] *= deadCells[k];
			dense_prev[k] *= deadCells[k];
		}
	}
	public boolean isDeadCell(int x, int y){
		return deadCells[ y*(N+2) + x+1 ] == 0;
	}
	// DIGL FUNCTIONS END
